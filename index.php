...
function buildCard(card, col) {
  ...
  if (col !== 'done') {
    ...
    actsHTML += `<button class="act-btn done" onclick="moveAPI(${card.id},'done');event.stopPropagation()">✓ 完成</button> <button class="act-btn" onclick="handleEditCardClick(event, ${card.id}, '${col}')">編輯</button> <button class="act-btn del" onclick="deleteAPI(${card.id});event.stopPropagation()">刪除</button></div>`;
  } else {
...
function handleEditCardClick(e, id, col) {
  if (e) {
    e.preventDefault();
    e.stopPropagation();
  }
