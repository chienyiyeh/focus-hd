...
<style>
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .board-wrap.has-annual { grid-template-columns: var(--annual-col-width, 280px) repeat(4, minmax(0, 1fr)); }

  /* 年度計畫欄 */
  --accent-annual: #B8860B;
  .col-annual { background: #FFFDF5; border: 2px solid #D4A843; border-radius: var(--radius-lg); overflow: hidden; transition: width 0.3s ease, min-width 0.3s ease; position: sticky; top: 72px; max-height: calc(100vh - 96px); display: flex; flex-direction: column; }
  .col-annual .col-accent { background: linear-gradient(90deg, #D4A843, #F0C060); height: 4px; }
  .col-annual .col-header { background: linear-gradient(135deg, #FFF8E7, #FFFDF5); border-bottom: 1px solid #E8C86A; flex-shrink: 0; }
  .col-annual .col-title { color: #7A5C00; }
  .col-annual .col-sub { color: #A07830; }
  .col-annual .badge-annual { background: #FFF0C0; color: #7A5C00; border: 1px solid #D4A843; }
  .col-annual .cards-area { overflow-y: auto; flex: 1; }

  /* 收折狀態 */
  .col-annual.collapsed { min-width: 40px; width: 40px !important; cursor: pointer; }
  .col-annual.collapsed .col-header,
  .col-annual.collapsed .cards-area,
  .col-annual.collapsed .annual-add-area { display: none; }
  .col-annual.collapsed .annual-collapsed-bar { display: flex; }
  .annual-collapsed-bar { display: none; flex-direction: column; align-items: center; justify-content: center; gap: 8px; height: 100%; min-height: 200px; padding: 16px 0; cursor: pointer; }
  .annual-collapsed-label { writing-mode: vertical-rl; text-orientation: mixed; font-size: 12px; font-weight: 700; color: #B8860B; letter-spacing: 1px; white-space: nowrap; }
  .annual-toggle-btn { background: none; border: none; cursor: pointer; font-size: 18px; padding: 4px; color: #B8860B; line-height: 1; }

  /* 年度計畫卡片 */
  .annual-card { background: var(--surface); border: 1px solid #E8C86A; border-radius: var(--radius); padding: 10px 12px; margin-bottom: 8px; position: relative; transition: box-shadow 0.15s; }
  .annual-card:hover { box-shadow: 0 2px 8px rgba(184,134,11,0.15); border-color: #D4A843; }
  .annual-card.type-year { border-left: 4px solid #D4A843; }
  .annual-card.type-month { border-left: 4px solid #85C1E9; margin-left: 8px; }
  .annual-card-header { display: flex; align-items: flex-start; gap: 6px; margin-bottom: 6px; }
  .annual-card-type { font-size: 9px; font-weight: 700; letter-spacing: 0.5px; padding: 2px 6px; border-radius: 8px; flex-shrink: 0; margin-top: 2px; }
  .annual-card-type.year { background: #FFF0C0; color: #7A5C00; }
  .annual-card-type.month { background: #D6EAF8; color: #1A5276; }
  .annual-card-title { font-size: 13px; font-weight: 600; color: var(--text); flex: 1; line-height: 1.4; }
  .annual-card-progress { display: flex; align-items: center; gap: 6px; margin-bottom: 6px; }
  .annual-progress-bar { flex: 1; height: 4px; background: #F0E0A0; border-radius: 2px; overflow: hidden; }
  .annual-progress-fill { height: 100%; background: linear-gradient(90deg, #D4A843, #F0C060); border-radius: 2px; transition: width 0.3s; }
  .annual-progress-text { font-size: 10px; color: #A07830; font-weight: 600; flex-shrink: 0; }
  .annual-card-checklist { margin-top: 6px; }
  .annual-checklist-item { display: flex; align-items: center; gap: 6px; padding: 3px 0; font-size: 11px; color: var(--text-secondary); border-bottom: 1px solid rgba(212,168,67,0.1); }
  .annual-checklist-item:last-child { border-bottom: none; }
  .annual-checklist-item input[type="checkbox"] { width: 13px; height: 13px; cursor: pointer; accent-color: #D4A843; flex-shrink: 0; }
  .annual-checklist-item.checked span { text-decoration: line-through; color: var(--text-muted); }
  .annual-card-actions { display: flex; gap: 4px; margin-top: 8px; justify-content: flex-end; }
  .annual-card-btn { font-size: 10px; padding: 3px 8px; border: 1px solid #E8C86A; background: var(--surface); border-radius: 6px; cursor: pointer; color: #A07830; font-family: inherit; }
  .annual-card-btn:hover { background: #FFF8E7; }
  .annual-card-btn.danger { border-color: #FCA5A5; color: #991B1B; }
  .annual-card-btn.danger:hover { background: #FEE2E2; }

  /* 年度計畫新增表單 */
  .annual-add-area { padding: 10px; border-top: 1px solid #E8C86A; background: #FFFDF5; flex-shrink: 0; }
  .annual-add-form { display: none; background: var(--surface); border: 1px solid #E8C86A; border-radius: var(--radius); padding: 10px; margin-bottom: 8px; }
  .annual-add-form.open { display: block; }
  .annual-type-select { display: flex; gap: 6px; margin-bottom: 8px; }
  .annual-type-btn { flex: 1; padding: 5px; border: 1.5px solid #E8C86A; background: var(--surface); border-radius: 6px; font-size: 11px; font-weight: 600; cursor: pointer; color: #A07830; font-family: inherit; text-align: center; }
  .annual-type-btn.active { background: #FFF0C0; border-color: #D4A843; color: #7A5C00; }
  .annual-input { width: 100%; padding: 7px 10px; border: 1px solid #E8C86A; border-radius: 6px; font-size: 13px; font-family: inherit; margin-bottom: 6px; outline: none; box-sizing: border-box; }
  .annual-input:focus { border-color: #D4A843; }
  .annual-form-btns { display: flex; gap: 6px; }
  .annual-save-btn { flex: 1; padding: 7px; background: #D4A843; color: white; border: none; border-radius: 6px; font-size: 12px; font-weight: 600; cursor: pointer; font-family: inherit; }
  .annual-save-btn:hover { background: #B8860B; }
  .annual-cancel-btn { padding: 7px 12px; background: var(--surface2); color: var(--text-secondary); border: 1px solid var(--border); border-radius: 6px; font-size: 12px; cursor: pointer; font-family: inherit; }
  ...
  .cornell-add-input:focus { border-color: var(--accent-focus); border-style: solid; }
  .cornell-add-btn { padding: 8px 12px; border: none; background: var(--accent-focus); color: white; border-radius: 6px; font-size: 14px; font-weight: 700; cursor: pointer; white-space: nowrap; flex-shrink: 0; }
  ...
  @media (max-width: 768px) { 
    ...
    /* 手機版年度欄：不 sticky，正常顯示 */
    .col-annual { position: static !important; max-height: none !important; }
    .col-annual.collapsed { display: none !important; }
...
<div class="main-wrap">
  ...
  <div class="board-wrap has-annual" id="board-wrap">

    <!-- 年度專案欄（最左，可收折） -->
    <div class="col col-annual" id="col-annual">
      <div class="col-accent"></div>
      <!-- 收折時顯示的直排標籤 -->
      <div class="annual-collapsed-bar" onclick="toggleAnnualCol()">
        <button class="annual-toggle-btn">▶</button>
        <span class="annual-collapsed-label">年度專案</span>
      </div>
      <!-- 展開時的 header -->
      <div class="col-header">
        <div class="col-title-row">
          <div class="col-title">🗺️ 年度專案</div>
          <div style="display:flex;align-items:center;gap:6px;">
            <span class="badge badge-annual" id="badge-annual">0 項</span>
            <button class="annual-toggle-btn" onclick="toggleAnnualCol()" title="收折" style="font-size:14px;">◀</button>
          </div>
        </div>
        <div class="col-sub">大目標 → 月里程碑 → 執行</div>
      </div>
      <div class="cards-area" id="cards-annual" style="padding:10px;"></div>
      <div class="annual-add-area">
        <div class="annual-add-form" id="annual-add-form">
          <div class="annual-type-select">
            <button class="annual-type-btn active" id="annual-type-year" onclick="selectAnnualType('year')">🏆 年目標</button>
            <button class="annual-type-btn" id="annual-type-month" onclick="selectAnnualType('month')">📅 月里程碑</button>
          </div>
          <input class="annual-input" id="annual-input-title" placeholder="標題..." onkeydown="if(event.key==='Enter')saveAnnualCard()">
          <textarea class="annual-input" id="annual-input-summary" placeholder="摘要（選填）..." rows="2" style="resize:none;"></textarea>
          <div class="annual-form-btns">
            <button class="annual-save-btn" onclick="saveAnnualCard()">儲存</button>
            <button class="annual-cancel-btn" onclick="closeAnnualForm()">取消</button>
          </div>
        </div>
        <button class="add-card-btn" style="border-color:#D4A843;color:#A07830;" onclick="openAnnualForm()">+ 新增目標 / 里程碑</button>
      </div>
    </div>
...
<div class="mobile-tab-bar">
  <button class="mobile-tab" data-col="annual" onclick="switchMobileTab('annual')">
...
function buildCard(card, col, cardNo) {
  ...
  if (hasChecklist) {
    ...
    card.checklist.forEach((item, idx) => {
      ...
      editableA += `<input type="checkbox" id="${cbId}" ${item.checked ? 'checked' : ''} onchange="inlineToggleChecklist(${cardIdStr}, ${idx}, '${col}', this); event.stopPropagation();" onclick="event.stopPropagation();">`;
  ...
  const editableB = `
    <div style="display:flex;gap:0;align-items:flex-start;height:100%;">
      ...
      <div class="mini-toolbar" id="mtb-${cardIdStr}"
        ...
        <!-- 文字色 -->
        <div style="position:relative;">
          <button id="color-btn-${cardIdStr}" class="mini-tb-btn" style="width:30px;height:30px;font-size:14px;padding:0;font-weight:900;border-bottom:3px solid #E24B4A;" onmousedown="toggleSubMenu('color-menu-${cardIdStr}',this);event.preventDefault();event.stopPropagation()">A</button>
          <div id="color-menu-${cardIdStr}" style="display:none;position:fixed;right:8px;background:#111110;border-radius:12px;padding:12px;flex-direction:row;gap:10px;z-index:200;box-shadow:0 4px 20px rgba(0,0,0,0.6);">
        ...
        <!-- 螢光筆 -->
        <div style="position:relative;">
          <button id="bg-btn-${cardIdStr}" style="width:30px;height:30px;background:transparent;border:none;cursor:pointer;padding:2px;" onmousedown="toggleSubMenu('bg-menu-${cardIdStr}',this);event.preventDefault();event.stopPropagation()">
          ...
          <div id="bg-menu-${cardIdStr}" style="display:none;position:fixed;right:8px;background:#111110;border-radius:12px;padding:12px;flex-direction:row;gap:10px;z-index:200;box-shadow:0 4px 20px rgba(0,0,0,0.6);">
...
// 勾選待辦
const _checklistSaveLock = new Set(); // 防止同一個 item 快速連點造成 race condition
async function inlineToggleChecklist(cardId, idx, col, checkboxEl) {
  const lockKey = `${cardId}-${idx}`;
  if (_checklistSaveLock.has(lockKey)) {
    // 正在儲存中，把 checkbox 恢復到鎖定前的狀態
    if (checkboxEl) checkboxEl.checked = !checkboxEl.checked;
    return;
  }
  _checklistSaveLock.add(lockKey);

  // 直接讀 checkbox 的當前值（browser 已翻轉），避免 state 讀取 race condition
  const newChecked = checkboxEl ? checkboxEl.checked : null;
  const found = getCard(cardId);
  if (!found) { _checklistSaveLock.delete(lockKey); return; }
  const checklist = JSON.parse(JSON.stringify(found.card.checklist || []));
  if (!checklist[idx]) { _checklistSaveLock.delete(lockKey); return; }
  checklist[idx].checked = newChecked !== null ? newChecked : !checklist[idx].checked;

  // 立即更新 DOM 樣式（不等 API 回應，讓使用者感覺即時）
  const cardEl = document.getElementById('card-' + cardId);
  if (cardEl) {
    ...
    if (checkboxEl) checkboxEl.checked = checklist[idx].checked;
  }

  await inlineSave(cardId, col, { checklist });
  _checklistSaveLock.delete(lockKey);
}
...
// 資料儲存在 localStorage（獨立於卡片 API）
const ANNUAL_KEY = 'focus_annual_cards_v1';
let annualCards = [];
let annualType = 'year'; // 'year' | 'month'
let annualCollapsed = false;

function loadAnnualCards() {
  try {
    const saved = localStorage.getItem(ANNUAL_KEY);
    annualCards = saved ? JSON.parse(saved) : [];
  } catch(e) { annualCards = []; }
}

function saveAnnualCards() {
  try { localStorage.setItem(ANNUAL_KEY, JSON.stringify(annualCards)); } catch(e) {}
}

// 收折/展開
function toggleAnnualCol() {
  annualCollapsed = !annualCollapsed;
  const col = document.getElementById('col-annual');
  const wrap = document.getElementById('board-wrap');
  if (annualCollapsed) {
    col.classList.add('collapsed');
    wrap.style.gridTemplateColumns = '40px repeat(4, minmax(0, 1fr))';
  } else {
    col.classList.remove('collapsed');
    wrap.style.gridTemplateColumns = '280px repeat(4, minmax(0, 1fr))';
  }
  try { localStorage.setItem('annual_collapsed', annualCollapsed ? '1' : '0'); } catch(e) {}
}

// 恢復收折狀態
function restoreAnnualColState() {
  try {
    const saved = localStorage.getItem('annual_collapsed');
    if (saved === '1') {
      annualCollapsed = true;
      document.getElementById('col-annual').classList.add('collapsed');
      document.getElementById('board-wrap').style.gridTemplateColumns = '40px repeat(4, minmax(0, 1fr))';
    }
  } catch(e) {}
}

// 新增表單
function openAnnualForm() {
  document.getElementById('annual-add-form').classList.add('open');
  document.getElementById('annual-input-title').focus();
}
function closeAnnualForm() {
  document.getElementById('annual-add-form').classList.remove('open');
  document.getElementById('annual-input-title').value = '';
  document.getElementById('annual-input-summary').value = '';
}
function selectAnnualType(type) {
  annualType = type;
  document.getElementById('annual-type-year').classList.toggle('active', type === 'year');
  document.getElementById('annual-type-month').classList.toggle('active', type === 'month');
}

function saveAnnualCard() {
  const title = document.getElementById('annual-input-title').value.trim();
  if (!title) { document.getElementById('annual-input-title').focus(); return; }
  const summary = document.getElementById('annual-input-summary').value.trim();
  const card = {
    id: Date.now(),
    type: annualType,
    title,
  ...
  annualCards.push(card);
  saveAnnualCards();
  closeAnnualForm();
  renderAnnualCards();
  toast('✅ ' + (annualType === 'year' ? '年目標' : '月里程碑') + '已新增');
}

function deleteAnnualCard(id) {
  if (!confirm('確定刪除？')) return;
  annualCards = annualCards.filter(c => c.id !== id);
  saveAnnualCards();
  renderAnnualCards();
}

// 待辦清單勾選
function toggleAnnualChecklist(cardId, idx) {
  const card = annualCards.find(c => c.id === cardId);
  if (!card || !card.checklist[idx]) return;
  card.checklist[idx].checked = !card.checklist[idx].checked;
  saveAnnualCards();
  renderAnnualCards();
}

// 新增子任務（行內輸入）
function addAnnualSubtask(cardId) {
  const input = document.getElementById('annual-sub-' + cardId);
  if (!input) return;
  const text = input.value.trim();
  if (!text) return;
  const card = annualCards.find(c => c.id === cardId);
  ...
  // 重新聚焦輸入框
  setTimeout(() => {
    const el = document.getElementById('annual-sub-' + cardId);
    if (el) el.focus();
  }, 50);
}

function deleteAnnualSubtask(cardId, idx) {
  const card = annualCards.find(c => c.id === cardId);
  if (!card) return;
...
function renderAnnualCards() {
  const area = document.getElementById('cards-annual');
  if (!area) return;
  area.innerHTML = '';

  if (annualCards.length === 0) {
    area.innerHTML = '<div class="empty" style="color:#A07830;">設定你的年度大目標<br>再拆成月里程碑</div>';
    document.getElementById('badge-annual').textContent = '0 項';
    return;
  }

  document.getElementById('badge-annual').textContent = annualCards.length + ' 項';
  ...
  annualCards.forEach(card => {
    ...
    const div = document.createElement('div');
    div.className = 'annual-card type-' + card.type;
    div.id = 'annual-card-' + card.id;

    const typeLabel = card.type === 'year' ? '🏆 年目標' : '📅 月里程碑';
    const typeClass = card.type === 'year' ? 'year' : 'month';

    let checklistHTML = '';
    if (total > 0) {
      checklistHTML = '<div class="annual-card-checklist">';
      card.checklist.forEach((item, idx) => {
        checklistHTML += `<div class="annual-checklist-item${item.checked ? ' checked' : ''}">
          <input type="checkbox" ${item.checked ? 'checked' : ''} onchange="toggleAnnualChecklist(${card.id}, ${idx}); event.stopPropagation();" onclick="event.stopPropagation();">
          <span>${escHtml(item.text)}</span>
          <button onclick="deleteAnnualSubtask(${card.id},${idx});event.stopPropagation()" style="background:none;border:none;color:#ccc;cursor:pointer;font-size:12px;margin-left:auto;padding:0 2px;flex-shrink:0;">✕</button>
        </div>`;
      });
      checklistHTML += '</div>';
    }
    ...
    const progressHTML = total > 0 ? `
      <div class="annual-card-progress">
        <div class="annual-progress-bar"><div class="annual-progress-fill" style="width:${pct}%"></div></div>
        <span class="annual-progress-text">${done}/${total}</span>
      </div>` : '';

    div.innerHTML = `
      <div class="annual-card-header">
        <span class="annual-card-type ${typeClass}">${typeLabel}</span>
        <span class="annual-card-title">${escHtml(card.title)}</span>
      </div>
      ${card.summary ? `<div style="font-size:11px;color:#A07830;font-style:italic;margin-bottom:6px;">${escHtml(card.summary)}</div>` : ''}
      ${progressHTML}
      ${checklistHTML}
      <div style="display:flex;gap:4px;margin-top:8px;align-items:center;">
        <input id="annual-sub-${card.id}" placeholder="新增子任務..." style="flex:1;border:1px dashed #D4A843;border-radius:6px;padding:4px 8px;font-size:11px;font-family:inherit;outline:none;background:var(--surface);"
          onkeydown="if(event.key==='Enter'){addAnnualSubtask(${card.id});event.preventDefault();}"
          onclick="event.stopPropagation()">
        <button onclick="addAnnualSubtask(${card.id});event.stopPropagation()" style="padding:4px 8px;background:#D4A843;color:white;border:none;border-radius:6px;font-size:12px;cursor:pointer;font-weight:700;">+</button>
        <button class="annual-card-btn danger" onclick="deleteAnnualCard(${card.id});event.stopPropagation()">🗑</button>
      </div>`;
...
// 手機版：年度欄也支援 switchMobileTab
const _origSetMobileTab = setMobileTab;
function setMobileTab(colName) {
  if (colName === 'annual') {
    document.querySelectorAll('.col').forEach(c => c.classList.remove('active'));
    document.querySelectorAll('.mobile-tab').forEach(t => t.classList.remove('active'));
    document.getElementById('col-annual').classList.add('active');
    const tab = document.querySelector(`.mobile-tab[data-col="annual"]`);
    if (tab) tab.classList.add('active');
    return;
  }
  _origSetMobileTab(colName);
}
