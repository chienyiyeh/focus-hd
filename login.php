...
// 有 remember_token cookie → 嘗試自動登入
if (isset($_COOKIE['remember_token'])) {
    ...
    try {
        ...
        if ($user) {
            ...
            $db->prepare("UPDATE remember_tokens SET expires_at = ? WHERE token = ?")->execute([$newExpires, $token]);
...
.error-message {
  background: #FFF3F2; border: 1px solid var(--error);
...
<div class="login-container">
  ...
  <form id="login-form">
    ...
    <div class="remember-row">
      ...
      <label for="remember_me">記住我（30 天自動登入）</label>
    </div>
...
loginForm.addEventListener('submit', async (e) => {
  ...
  if (!username || !password) { showError('請輸入帳號和密碼'); return; }
